...
class ImportValidationUnitTest {
    @Test(expected = ImportException::class)
    fun importWithEmptyJson() {
        ImportValidation.validate(jacksonObjectMapper().readTree(""))
    }
    ...
    fun importWithValidSettings() {
        ImportValidation.validateRepositorySettings(
            jacksonObjectMapper().readTree(
                """          
                {
                  "tabRowChance": 0,
    ...
    fun importWithInvalidBag() {
        ImportValidation.validateRepositoryBag(
            jacksonObjectMapper().readTree(
                """          
                {
                    "dice": [
    ...
    fun importWithValidRoll() {
        ImportValidation.validateRepositoryRoll(
            jacksonObjectMapper().readTree(
                """          
                {
                  "values": {
